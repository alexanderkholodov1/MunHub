...
- **Documentation matrix:** `AGENTS.md` guardrail 7 (the table). Changelog fragments in
  `changelog.d/` per `changelog.d/README.md`.
- **Entry points to check:** `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`,
  `.cursor/rules/00-agents.mdc`, `.github/copilot-instructions.md`.
- **Special truth requirements:** `docs/research/THEORETICAL-FOUNDATION.md` — code and docs
  may never contradict it (scientific claims need citations); `docs/STATUS.md` is
  orchestrator-owned and must reflect the PR queue; `public/` is a read-only v5 reference
  (changes there outside consultation are findings).
- **Language policy:** English everywhere; es/pt-BR only as UI translation files.
