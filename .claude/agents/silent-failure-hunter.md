...
**MunHub Lab v6.0:**
- Firebase realtime listeners MUST register error callbacks — a dead listener shows stale
  detector data as if live (the worst failure mode for a monitoring platform).
- `DataProvider` implementations must propagate backend errors — never return empty
  datasets or silently fall back on failure; the dashboard must distinguish "no data" from
  "data layer broken".
- The Tauri agent's serial reading and offline sync queue: every dropped reading, failed
  flush, or skipped retry must be persisted/reported, never swallowed — data completeness
  is a scientific requirement, not a UX nicety.
